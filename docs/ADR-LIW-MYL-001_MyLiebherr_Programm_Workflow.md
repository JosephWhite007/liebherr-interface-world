...
- **S7 · MYL-WALLET-ADP Adapter = `CoreBridge\WalletBridge`. ✅ UMGESETZT (alpha.109/117, lesend)** — Schreib-/Token-Mehrwährung deferred (Abschnitt 9). **Keine eigene Ledger-Tabelle im Satelliten.** Neuer Bridge ruft `Araliya\Platform\Core\Modules\Wallet\WalletService` (`class_exists`-guarded): `get_balance`/`get_summary`/`get_transactions` (lesen), `reserve`/`confirm_reservation`/`release_reservation` (zwei-Phasen-Buchung, EUR), `debit`/`credit` (mit caller-eigenem `idempotency_key`); beobachtet `EventBus` `WalletDebited`/`WalletCredited`. **Token-Währungszweig + Ablösung `Adventures\TokenAccount` an das Wallet-Pflichtenheft gekoppelt (Core-Kategorie A, Abschnitt 9) — nicht in diesem Programm.** *Abnahme:* SEC 02, SEC 03 (über WalletService-Idempotenz/-Status), MYL 004 (Salden aus WalletService).
- **S8 · MYL-WALLET UI. ✅ UMGESETZT (alpha.117, read-only).** `[liw_my_wallet]`: Salden-Karten (Saldo/gutgeschrieben/belastet/Budget) + letzte Buchungen als Belege über `WalletBridge` (Core `get_summary`/`get_transactions`). *Abnahme:* MYL 004/006 (`LIW_ABNAHME.md §9`). *Offen:* verfügbar/reserviert/gesperrt-Buckets + Export/Pagination (Wallet-Pflichtenheft).
- **S9 · MYL-PTIME Zeitmessung (serverautoritär). ✅ UMGESETZT (alpha.113).** `PlatformTime\SessionClock` auf Basis SessionService/SessionMeter; Heartbeat, Pause bei Inaktivität/Austritt. *DB:* `ary_liw_ptime_session`. *REST:* `GET status`, `POST heartbeat`, `POST stop`. *Abnahme:* **MYL 026**. *Flag:* `liw_ptime_enabled`.
- **S10 · MYL-PTIME Abrechnung & Naht (Reservieren→Bestätigen). ✅ UMGESETZT (alpha.113).** Zeit→Token via versionierter Zeit-Tokenregel; append-only `ary_liw_ptime_charge` (satellitenseitiger Nachweis, **kein** Saldo). Beim Eintritt `WalletBridge::reserve()`, bei Stop `confirm_reservation()`, bei Abbruch `release_reservation()` — **nur** bei `liw_ptime_charge_live`; sonst wird der Abschnitt lediglich als *ausstehende* Token-Abrechnung protokolliert (Token-Buchung wartet auf den Mehrwährungs-Wallet aus dem Wallet-Pflichtenheft). Zustandsautomat started→…→settled/voided. *Abnahme:* **MYL 027, MYL 028**, SEC 02/03.
- **S11 · MYL-PTIME Schachuhr-Widget. ✅ UMGESETZT (alpha.113)** — schwebend unten links, nur angemeldet + `liw_myl_access`. Schwebendes, jederzeit ein-/ausblendbares Widget (Muster Emergency/Favicon) mit Zeit + laufenden Tokenkosten; `prefers-reduced-motion`; Tastatur. *Abnahme:* **MYL 025**, MYL 012.
...
- **S13 · MYL-ADV Own Adventures. ✅ UMGESETZT (alpha.121, Wiederverwendung Insel)** — `[liw_my_adventures]` eigene Beiträge, Status-Filter, Maschine/Bauteil+Token; Lifecycle/Kauf bleiben in der Insel. Status draft→…→archived (§19), Kauf reservieren→liefern→buchen, Lizenz-Reopen ohne Neubuchung. *REST:* `POST /adventures`, `/submit`, `/purchase`, `?ownership=own`. *Abnahme:* MYL 005, MYL 007, MYL 008, MYL 016.
- **S14 · MYL-ADV Drei-Wort-Name. ✅ UMGESETZT (alpha.130)** — `MyLiebherr\ThreeWordLabel` (Tabelle three_word_label): normiert Maschine·Problem·Handlung, Vorschlag→Bestätigung in Own Adventures, Synonyme + Agentensuche (`search`), REST label GET/PUT. Muster Maschine-Problem-Handlung, genau drei normalisierte Begriffe, Synonyme/Übersetzungen im Index. *DB:* `ary_liw_myl_three_word_label`. *Abnahme:* MYL 017.
- **S15 · MYL-DREAMS. ✅ UMGESETZT (alpha.121, voll)** — `[liw_my_dreams]`, Tabelle `dream_item`, Bild/Maschinenbezug/3-Wort-Titel/Notiz/Tags/Sammlung/Cover/Wunschstatus/Reihenfolge. Maschinen-Bilderbuch, privat; Drei-Wort-Titel, Notiz, Tags, Reihenfolge, Cover, Wunschstatus. *DB:* `ary_liw_myl_dream_item`. *REST:* `GET/POST /dreams`. *Abnahme:* MYL 014.
- **S16 · MYL-GALLERY. ✅ UMGESETZT (alpha.121)** — `[liw_my_gallery]`, Tabelle `gallery_item`, Mediathek-ID/Titel/Album/Tags/Sichtbarkeit. Private Galerie: Upload/Import/Alben/Metadaten/Rechteprüfung/Archiv. *DB:* `ary_liw_myl_gallery_item`. *REST:* `GET/POST /gallery`. *Abnahme:* MYL 015 (privat), Upload-Prüfung SEC 04.
- **S17 · CVF-6CARD Migration. ✅ UMGESETZT (alpha.121)** — my_liebherr + pocket_information additiv/inaktiv im Board-Seed. Board 4→6 Karten, neue stabile moduleIds `my_liebherr`, `pocket_information`, **initial deaktiviert**; bestehende Viererflows verlustfrei. *Datei:* `src/Cvf/Schema.php`, `BoardSchema.php`, Board-UI. *Abnahme:* MYL 023, CVF „Schema Migration".
...
- **S20 · MYL-SHARE. ✅ UMGESETZT (alpha.121, vorgezogen)** — `share_grant`, Kollegen `active` / World `pending` (Review offen), Widerruf; `[liw_shared_colleagues]`/`[liw_shared_world]`. Shared with Colleagues / World mit getrennten, nachvollziehbaren Grants (Empfänger/Ablauf/Kommentar/Download/Widerruf; World zusätzlich Review). *DB:* `ary_liw_myl_share_grant`. *REST:* `POST/DELETE /gallery/{id}/shares`. *Abnahme:* MYL 015 (drei Sichten).
- **S21 · MYL-CONTACTS. ✅ UMGESETZT (alpha.124)** — `[liw_my_contacts]`, ContactState/ContactRepository, Anfrage→Connection→Service-Exchange (Buchung als Hook `liw_myl_service_charge`, deferred); Tabellen contact_request/connection/service_exchange. Kontaktanfrage → Zustimmung → Contact Connection (§33), Service Exchange erzeugt genau eine Buchung; Verbindung beenden. *DB:* `ary_liw_myl_contact_request`, `ary_liw_myl_wallet_connection`, `ary_liw_myl_service_exchange`. *REST:* `POST /contacts/requests`, `/decision`, `POST /connections/{id}/services`. *Abnahme:* MYL 018, MYL 019, MYL 020, Sicherheitsgrenze §33.
...
- **S23 · POCKET (6. Reiter). ✅ UMGESETZT (alpha.124)** — Modul `src/Pocket/`, `[liw_pocket]`, REST `pocket/v1` (feed/items/ack), Pflichtquittierung, Reiter im WorldSwitcher, Seeder `/pocket-information/`. **+ alpha.128: regelbasierte Auto-Infos** (`PocketRules`/`FeedService`: My Briefing/Tasks/Machine/Adventure, erklärbar, nicht persistiert, `liw_pocket_rules_enabled` abschaltbar). Briefing/Alerts/Tasks/Saved/Machine/Adventure Pocket; regelbasiert + erklärbar; Pflichtquittierung getrennt protokolliert; Return Routes. *DB:* `ary_liw_pocket_item`. *REST:* `GET /pocket/v1/feed`, `POST /pocket/v1/items/{id}/ack`. *Abnahme:* MYL 021, MYL 022. *Flag:* `liw_pocket_enabled`.
- **S24 · MYL-MACHINES ✅ (alpha.124)** (`[liw_my_machines]`, Tabelle machine) **· CVF-6CARD Teil 2 ✅ (alpha.132)** — `BoardRepository::wire_module_card()` + Seeder `liw-seed-cvf-6cards.php`: First-Entry/Return-Route-Simulation von my_liebherr + pocket_information laufen im Board (MYL 013 ✅). **R5-Vollabnahme kontextübergreifend** bleibt Pilot-Sache (R6). Ursprünglich: My Machines mit Local-Intelligence-Bezug; First Entry + Return Route beider neuer CVF-Karten laufen vollständig in der Simulation; Direktlink-Schutz; Publish/Diff/Rollback deterministisch. *Abnahme:* MYL 013 (vollständig), CVF „Board Darstellung/Simulation/Runtime/Versionierung". *Exit R5:* kontextübergreifende Abnahme grün.
