...
- **S23 · POCKET (6. Reiter). ✅ UMGESETZT (alpha.124)** — Modul `src/Pocket/`, `[liw_pocket]`, REST `pocket/v1` (feed/items/ack), Pflichtquittierung, Reiter im WorldSwitcher, Seeder `/pocket-information/`. Briefing/Alerts/Tasks/Saved/Machine/Adventure Pocket; regelbasiert + erklärbar; Pflichtquittierung getrennt protokolliert; Return Routes. *DB:* `ary_liw_pocket_item`. *REST:* `GET /pocket/v1/feed`, `POST /pocket/v1/items/{id}/ack`. *Abnahme:* MYL 021, MYL 022. *Flag:* `liw_pocket_enabled`.
